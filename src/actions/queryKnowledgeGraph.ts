...
export const queryKnowledgeGraphAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        ...
        const queryTerms = [
            'scf energy', 'homo lumo', 'b3lyp', 'dft', 'quantum', 'molecular',
            'frequency', 'optimization', 'basis set', 'gaussian', 'calculation',
            'dipole moment', 'geometry', 'orbital', 'electron'
        ];
        
        const text = content.text?.toLowerCase() || content.query?.toLowerCase() || '';
    ...
    description: "Query the quantum chemistry knowledge graph for calculation data and molecular properties",
    handler: async (
        runtime: IAgentRuntime,
        message: Memory,
        state: State,
        ...
        try {
            ...
            const result = await queryQuantumChemistryData(
                runtime,
                query,
                format
            );
            ...
            if (result.success) {
                ...
                if (result.data?.results && Array.isArray(result.data.results)) {
                    result.data.results.slice(0, 5).forEach((item: any, index: number) => {
                        responseText += `${index + 1}. ${item.title || item.name || 'Calculation'}\n`;
                        if (item.scf_energy) responseText += `   SCF Energy: ${item.scf_energy} eV\n`;
                        if (item.method) responseText += `   Method: ${item.method}\n`;
                        if (item.basis_set) responseText += `   Basis Set: ${item.basis_set}\n`;
                        responseText += '\n';
                    });
