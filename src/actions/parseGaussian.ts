...
export const parseGaussianAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        ...
        const isGaussianFile = (content.url?.includes('.log') || false) || 
                              (content.url?.includes('.out') || false) ||
                              textContent.includes('gaussian');
        
        return hasUrl && isGaussianFile;
    },
    description: "Parse a Gaussian 16 quantum chemistry logfile and import into knowledge graph",
    handler: async (
        runtime: IAgentRuntime,
        message: Memory,
        state: State,
